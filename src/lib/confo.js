...
var confo_variables = {
    ...
    isSpotLightM: false,
    isSpotLightP: false,
    isSpotLightBeforeAnnotate: false,
    isAudioMute: false,
    isAnnotate: false,
    before_annotate_id: '',
    annotateStreamID: '',
    ...
    streamShare: null,
    cobrowsingUrls: [['Enablex Home Page', 'https://meet.vcloudx.com'], ['Enablex Cpaas', 'https://www.enablex.io/cpaas/'], ['Enablex Ucaas', 'https://www.enablex.io/ucaas/'], ['Enablex Video-Api', 'https://www.enablex.io/cpaas/video-api'], ['Enablex Video Embed', 'https://www.enablex.io/cpaas/video-embed/']],
    VideoSize: {
        'HD': [320, 180, 1280, 720],
        'SD': [640, 480, 640, 480],
        'LD': [80, 45, 640, 360],
    ...
    ConnectCall: function (token, cb) {
        ...
        localStream = EnxRtc.joinRoom(token, {
            ...
            if (success && success !== null) {
                ...
                confo_variables.roomName = success.roomData.name;
                confo_variables.conf_num = success.roomData.conf_num;
                document.title = success.roomData.name;

                //Cobrowsing url update in confo.html
                var cobrowse_url_select = document.querySelector('#cobrowse_urls');
                confo_variables.cobrowsingUrls.forEach((item) => {
                    var cobrowse_url_options = document.createElement('option');
                    cobrowse_url_options.value = item[1];
                    cobrowse_url_options.innerHTML = item[0];
                    cobrowse_url_select.appendChild(cobrowse_url_options);
                })
                ...
                if (!isModerator) {
                    ...
                    document.querySelector('#co_browsing_button_parent').style.display = 'none';
                }
                    ...
                    if (room.roomSettings.hasOwnProperty('canvas')) {
                        {
                            if (room.roomSettings.canvas === true) {
                                ...
                                annotate_div.setAttribute('id', `a_self-view`);
                                ...
                                annotate_div.setAttribute('title', 'Annotate');
                                spot_and_annotate.appendChild(annotate_div);
                            }
                        }
                    }
                    video_caption.appendChild(spot_and_annotate);
                ...
                room.addEventListener('active-talkers-updated', function (event) {
                    ...
                    if (event.message && event.message !== null && event.message.activeList && event.message.activeList !== null) {
                        ...
                        if (ATList.length === 0 && document.querySelectorAll('.remote-view').length === 0) {
                            ...
                            if (confo_variables.isAnnotate === true && confo_variables.annotateClientId === room.clientId) {
                                confo_variables.previousToggleClass = 'custom11';
                            }
                            document.querySelector('.custom-multi-app-page .video-area .row-fluid').setAttribute('class', 'row-fluid custom1 custom11');
                            return;
                        }
                    ...
                    ATList.forEach((item, index) => {
                        ...
                        confo_variables.activeTalkerInfo[ATList[index].streamId] = { 'name': ATList[index].name, 'clientId': ATList[index].clientId, 'mediatype': ATList[index].mediatype, 'deviceType': ATList[index].deviceType };
                    });
                    ...
                    difference.forEach((item, index) => {
                        if (ATList_id.indexOf(item) === -1) {
                            ...
                            if (!st.local) {
                                ...
                                icon_div.setAttribute('class', 'icon_div');
                                ...
                                icon_div.appendChild(video_icon);
                                if (isModerator) {
                                    var spot_and_annotate = document.createElement('div');
                                    spot_and_annotate.setAttribute('class', 'spotannotate');
                                    var spot_div = document.createElement('div');
                                    ...
                                    // video_caption.appendChild(spot_and_annotate);
                                    icon_div.appendChild(spot_and_annotate);

                                }
                                video_caption.appendChild(icon_div);
                                ...
                                if (confo_variables.activeTalkerInfo[item].deviceType === 'mobile') {
                                    document.querySelector(`#stream${item}`).style.objectFit = 'contain';
                                }
                    ...
                    ATList.forEach((item, index) => {
                            ...
                            if (room.clientId !== item.clientId) {
                                ...
                                if (room.me.role === 'moderator') {
                                    if (confo_variables.isShareReceived === true) {
                                        ...
                                        document.querySelector(`#s_${item.streamId}`).setAttribute('title', 'Spotlight');
                                    }
                                    ...
                                    } else if (confo_variables.isAnnotate === true) {
                                        document.querySelector(`#s_${item.streamId}`).setAttribute('onclick', 'spotlight(this)');
                                        document.querySelector(`#s_${item.streamId}`).classList.replace('remove-spotlight', 'spotlight');
                                    ...
                                    if (confo_variables.isShareReceived === true || confo_variables.isAnnotate === true) {
                        ...
                        else if (item.spotlight === true && confo_variables.isSpotLightM === false) {
                ...
                room.addEventListener("user-disconnected", function (event) {
                    ...
                    if (room.userList.size > 1 && isModerator) {
                        document.querySelector('#co_browsing_button_parent').style.display = 'block';
                    } else {
                        document.querySelector('#co_browsing_button_parent').style.display = 'none';
                    }
                    if (room.me.role === 'moderator') {
                        ...
                        //New code 
                        if (confo_variables.isSpotLightM && confo_variables.isAnnotate) {
                            if (confo_variables.SpotLightClientId === event.clientId && confo_variables.annotateClientId === event.clientId) {
                                ...
                                document.querySelector('.custom-app-wrapper').classList.remove('screen-open');
                                document.querySelector('.custom-app-wrapper').classList.remove('spotlight-open');
                                document.querySelector('.custom-app-wrapper').classList.remove('annotate-open');
                                ...
                                confo_variables.SpotLightUserStreamId = '';
                                confo_variables.SpotLightClientId = '';
                            } else if (confo_variables.SpotLightClientId === event.clientId) {
                                confo_variables.isSpotLightM = false;
                                confo_variables.SpotLightUserStreamId = '';
                                confo_variables.SpotLightClientId = '';
                            } else if (confo_variables.annotateClientId === event.clientId) {
                                room.stopAnnotation(function (res) {
                                    // console.log("stopAnnotation", JSON.stringify(res));
                                    if (res.result == 0) {
                                        // $("#share_screen_btn").prop("title", "Start Share").removeClass('blink-image');
                        ...
                        } else if (confo_variables.isSpotLightM) {
                            if (confo_variables.SpotLightClientId === event.clientId) {
                                document.querySelector('.custom-app-wrapper').classList.remove('screen-open');
                                document.querySelector('.custom-app-wrapper').classList.remove('spotlight-open');
                        ...
                        } else if (confo_variables.isAnnotate) {
                    ...
                    //
                        ...
                        //New Code
                        if (confo_variables.isSpotLightP && confo_variables.isAnnotate) {
                            if (confo_variables.SpotLightClientId === event.clientId && confo_variables.annotateClientId === event.clientId) {
                        ...
                        } else if (confo_variables.isSpotLightP) {
                            if (confo_variables.SpotLightClientId === event.clientId) {
                                document.querySelector('.custom-app-wrapper').classList.remove('screen-open');
                                document.querySelector('.custom-app-wrapper').classList.remove('spotlight-open');
                                var r = document.querySelector(`.remote_view_${confo_variables.SpotLightUserStreamId}`);
                ...
                room.addEventListener('canvas-started', function (event) {
                    if (room.clientId !== event.message.clientId) {
                        ...
                        if (st.stream !== undefined) {
                            if (confo_variables.isSpotLightP) {
                                document.querySelector('.custom-app-wrapper').classList.remove('spotlight-open');
                        ...
                        confo_variables.annotateClientId = event.message.clientId;
                    }
                })
                ...
                room.addEventListener("canvas-stopped", function (event) {
                    if (room.clientId !== event.message.clientId) {
                        if (confo_variables.isSpotLightP) {
                            ...
                            if (event.message.clientId !== confo_variables.SpotLightClientId && confo_variables.SpotLightClientId != '') {
                                document.querySelector('.custom-app-wrapper').classList.add('spotlight-open');
                                var scr = document.querySelector('.screen-inner');
                ...
                room.addEventListener('fs-download-result', function (event) {
                    ...
                    switch (evtMsg.messageType) {
                        case 'download-available':
                            ...
                            var message = document.createElement('div');
                            message.setAttribute('class', 'message');
                            message.innerHTML = `<p>${sender} shared a file <br/> File: ${filename}<br/> <a href="javascript:void(0);" style="font-size:inherit;" onclick="file_download(${job_id})">[Download]</span>}</p>`;
                            desc.appendChild(message);
                            var time_div = document.createElement('div');
                            time_div.setAttribute('class', 'time');
                            time_div.setAttribute('style', 'font-size: smaller');
                ...
                room.addEventListener('user-data-received', function (resp) {
                    ...
                    if (resp.message.message.type === 'cobrowsing') {
                        window.open(resp.message.message.message.url);
                    }
                });
    ...
    unmuteLocalAudio: function () {
        localStream.unmuteAudio(function (res) {
            if (res.result === 0) {
                ...
                toastr.options.timeOut = 80000000;
                toastr.options.positionClass = 'toast-top-right';
    ...
    throwAnnotationToLargeScreen: function (id_annotate_div) {
        document.querySelector('.custom-app-wrapper').classList.add('screen-open');
        document.querySelector('.custom-app-wrapper').classList.add('spotlight-open');


        var scr = document.querySelector('.screen-inner');
        if (id_annotate_div == 'self-view') {
            var r = document.querySelector(`.item1`);
        } else {
            var r = document.querySelector(`.remote_view_${parseInt(id_annotate_div)}`);
        }
        scr.appendChild(r);
        // console.log("screen-inner", scr);
        confo_variables.layoutSync();
    ...
    throwAfterAnnotation: function (id_annotate_div) {
        document.querySelector('.custom-app-wrapper').classList.remove('screen-open');
        document.querySelector('.custom-app-wrapper').classList.remove('spotlight-open');
        document.querySelector('.custom-app-wrapper').classList.remove('annotate-open');
        if (id_annotate_div != 'self-view') {
            var r = document.querySelector(`.remote_view_${id_annotate_div}`);
        } else {
            var r = document.querySelector(`.item1`);
        }
        var fluid = document.querySelector('.row-fluid');
        ...
        confo_variables.layoutSync();
        // document.getElementById(`s_${id_annotate_div}`).style.display = 'block';
    },
    annotateUser: function (_this) {
        ...
        else if (this.isSpotLightM === true) {
        ...
        if (id_annotate_div != 'self-view') {
            ...
            this.annotateStreamID = localStream;
        }
        ...
        if (id_annotate_div != 'self-view') {
            ...
            this.annotateClientId = localStream.clientId;
            document.querySelector(`#a_self-view`).style.zIndex = 5010;
            // document.querySelector(`#s_self-view`).style.zIndex = 5010;
        }
        ...
        this.adjustAnnotationCanvas(id_annotate_div);
        this.before_annotate_id = _this.id;
        this.isAnnotate = true;
        document.getElementById(_this.id).setAttribute('title', 'Remove Annotate');
    ...
    adjustAnnotationCanvas: function (id_annotate_div) {
        var mycanvas = document.getElementById(`draw_veneer2`);
        var canvas = mycanvas.getContext("2d");
        var rect = mycanvas.getBoundingClientRect();
        var ctx = canvas.getImageData(0, 0, mycanvas.width, mycanvas.height);

        var height = 0;
        var width = 0;
        if (id_annotate_div != 'self-view') {
            height = document.querySelector(`.remote_${confo_variables.activeTalkerInfo[parseInt(id_annotate_div)].clientId}`).clientHeight;
            width = document.querySelector(`.remote_${confo_variables.activeTalkerInfo[parseInt(id_annotate_div)].clientId}`).clientWidth;
        } else {
            height = document.querySelector(`.item1`).clientHeight;
            width = document.querySelector(`.item1`).clientWidth;
        }
        ...
        if (id_annotate_div != 'self-view') {
            ...
            document.querySelector(`#draw_veneer2`).setAttribute('height', `${height}`);
            document.querySelector(`#stream${parseInt(localStream.getID())}_veneer`).setAttribute('height', `${height}`);
            document.querySelector(`#draw_veneer2`).setAttribute('width', `${width}`);
            document.querySelector(`#stream${parseInt(localStream.getID())}_veneer`).setAttribute('width', `${width}`);
        }
        // canvas.putImageData(ctx,rect.left,rect.top);
        createImageBitmap(ctx).then(function (imgBitmap) {
    ...
    stopAnnotation: function (_this) {
        ...
        document.getElementById(_this.id).setAttribute('title', 'Annotate');
        document.getElementById(_this.id).classList.replace('remove-annotate', 'annotate');


    },
    getRoom: function() {
        return confo_variables.room;
    },
    ...
    copy: function () {
        ...
        let val = window.location.origin + "?invite=" + this.roomId;
        if (this.appId !== '') {
            val += '-' + this.appId;
        }
        inp.value = val;
    ...
    sendData: function (data) {
        // var encoded_room_name = this.roomName.split(" ").join("");
        data.message.url = data.message.url + `?cobrowse_name=${this.conf_num}`
        room.sendUserData(data, true, [], function (resp) {
            window.open(data.message.url);
...
$('body').on('click', '#file_upload_public', function(e){
    e.target.value = null;
})

$('body').on('change', '#file_upload_public', function(e){
    handlePublicFileSharing(e)
})
...
//new ResizeObserver(outputsize).observe(document.querySelector('#screen_share'));
...
window.addEventListener('resize', function (event) {
    ...
    // if (!navigator.userAgentData.mobile) {
        if (confo_variables.isAnnotate === true) {
            if (confo_variables.annotateStreamID.elementID == 'self-view') {
                confo_variables.adjustAnnotationCanvas('self-view');
            } else if(confo_variables.annotateStreamID.config.streamID !== undefined) {
                confo_variables.adjustAnnotationCanvas(confo_variables.annotateStreamID.config.streamID);
            }
        // }
    }
...
function spotlight(_this) {
    var id_annotate_div = _this.id.replace('s_', '');

    if (confo_variables.isSpotLightM === true) {
        toastr.options.positionClass = 'toast-bottom-right';
        toastr.error('Another participant is spotlight');
    }
    // else if (confo_variables.isAnnotate === true && id_annotate_div != confo_variables.annotateStreamID.config.streamID) {

    //     toastr.options.positionClass = 'toast-bottom-right';
    //     toastr.error('Only same annotated can be spotlight');
...
function annotate(_this) {
    ...
    if (confo_variables.isShareReceived || confo_variables.isShareStarted) {
        toastr.options.positionClass = 'toast-bottom-right';
        toastr.error('Screen Share is on');
    }
    // else if (confo_variables.isSpotLightM === true && id_annotate_div != confo_variables.SpotLightUserStreamId) {
    //     toastr.options.positionClass = 'toast-bottom-right';
    //     toastr.error('Only same spotlight can be annotated');
    // }
    else if (confo_variables.isAnnotate === true) {
        toastr.options.positionClass = 'toast-bottom-right';
        toastr.error('Annotation is on');
        ...
        if (confo_variables.isSpotLightM === false) {
            // if(document.getElementById(id_annotate_div).style.display === 'block') 
            // {
            confo_variables.throwAnnotationToLargeScreen(id_annotate_div);
            confo_variables.isSpotLightBeforeAnnotate = false;

            // document.getElementById(`s_${id_annotate_div}`).style.display = 'none';
            // }
        } else {
            // document.getElementById(`s_${id_annotate_div}`).style.display = 'none';
            if (id_annotate_div != confo_variables.SpotLightUserStreamId) {
                var r = document.querySelector(`.remote_view_${confo_variables.SpotLightUserStreamId}`);
                var fluid = document.querySelector('.row-fluid');
                fluid.appendChild(r);
                confo_variables.layoutSync();
            ...
            confo_variables.isSpotLightBeforeAnnotate = true;
        }
        confo_variables.annotateUser(_this);
    }
    document.querySelector('.custom-app-wrapper').classList.add('annotate-open');
...
function removeAnnotation(_this) {
    ...
    if (confo_variables.isSpotLightM === true) {
        if (id_annotate_div != confo_variables.SpotLightUserStreamId) {
            ...
            confo_variables.layoutSync();
            confo_variables.throwAnnotationToLargeScreen(confo_variables.SpotLightUserStreamId);
        }
        confo_variables.isSpotLightBeforeAnnotate = true;
    } else {
        document.querySelector('.custom-app-wrapper').classList.remove('annotate-open');
        confo_variables.isSpotLightBeforeAnnotate = false;
        confo_variables.throwAfterAnnotation(id_annotate_div);
    }
    if (confo_variables.isSpotLightBeforeAnnotate === false) {
        // confo_variables.throwAfterAnnotation(id_annotate_div);
    }
    document.querySelector('.custom-app-wrapper').classList.remove('annotate-open');
}
...
function startCobrowsing() {
    var url = document.querySelector('#cobrowse_urls').value;
    var data = {
        "type": "cobrowsing",
        "message": {
            "url": `${url}`,
